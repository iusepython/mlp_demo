function [W1,b1] = sparse_autoencoder(X,Y,hidden_size,lambda,beta,rho,max_iter,activation_type)
    d1 = size(X,2);
    d2 = hidden_size;
    d3 = size(Y,2);
    % init
    ...
    options.display = 'on';
    [opttheta, loss] = minFunc(@(theta) sparseAutoencoderLoss(theta,d1,d2,d3,xs,ys,lambda,beta,rho,activation_type), theta,options);
...
% loss function
function [loss,grad] = sparseAutoencoderLoss(theta,d1,d2,d3,xs,ys,lambda,beta,rho,activation_type)
    ...
    rho_hat = mean(a2,1);
    delta2 = (delta3*Ws{2}'+beta*repmat(-rho./rho_hat+(1-rho)./(1-rho_hat),size(a2,1),1)).*grad_activation(z2, activation_type); % sparse regularization
    ...
    partial_Ws{1} = (xs'*delta2)/size(xs,1)+lambda*Ws{1};
    ...
    KL_divergence = sum(rho.*log(rho./rho_hat)+(1-rho).*log((1-rho)./(1-rho_hat)));
    loss = sum(0.5*sum((ys-h).^2,2))/size(ys,1)+lambda/2*W2sum+beta*KL_divergence;
end
