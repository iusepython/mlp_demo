...
%% load data
trainX = loadMNISTImages('./MNIST/train-images.idx3-ubyte');
trainLabel = loadMNISTLabels('./MNIST/train-labels.idx1-ubyte');
trainY = zeros(size(trainLabel,1),10);
for i = 1:size(trainLabel,1)
    label = trainLabel(i);
    trainY(i,label+1)=1;
end
testX = loadMNISTImages('./MNIST/t10k-images.idx3-ubyte');
testLabel = loadMNISTLabels('./MNIST/t10k-labels.idx1-ubyte');
